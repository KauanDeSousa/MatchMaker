...
export default function DetalhePartida({ params }: { params: { value: string } }) {
    ...
    const fetchPartida = async () => {
        const id = JSON.parse(params.value).id;
    ...
    const registrarEvento = async () => {
        ...
        try {
            const response = await fetch(`/api/partidas/${id}/eventos`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
